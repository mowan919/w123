...
class ApiPermissionCode(StrEnum):
    ...
    #: 读取审计日志（`08 §8`：`GET /audit/logs*`、`GET /traces*`）。
    #:
    #: 为什么**不**对日志应用数据范围（INTERIM-10-01）
    #: --------------------------------------------
    #: `10 §10` 的数据范围是围绕"业务记录属于哪个部门"定义的，
    #: 而五张日志表**没有** `department_id` 列 —— 审计记录天然是跨部门的
    #: （一次请求可能同时涉及多个部门的资源）。给日志发明一套
    #: "按操作者所属部门过滤"的范围等于**削弱**审计：
    #: 跨部门操作（恰恰是最需要看见的那些）会被过滤掉。
    #:
    #: 因此日志的可见性**只**由本权限位承担，范围过滤不适用。
    #: 这在安全上的代价是"能查审计的人看得见全部审计"，
    #: 该代价由"权限位必须显式授予"来约束，而不是靠范围再切一刀。
    AUDIT_READ = "AUDIT_READ"
    #: 读取链路追踪（`08 §8`：`GET /traces*`）。
    #:
    #: 与 `AUDIT_READ` 分开，理由同 `PARAM_MANAGE` 与 `DICT_MANAGE` 的分离：
